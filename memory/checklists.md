...
- [ ] `knowledge-mcp search("тема")` — проверить по базе знаний
...
## Перед записью в Pack (MCP-верификация)

- [ ] `knowledge-mcp search("тема кандидата")` → нет дубликата
- [ ] `knowledge-mcp get_document("целевой файл")` → нет противоречия с существующим
- [ ] Если guides-контент → `knowledge-mcp search(query, source_type="guides")` → нет пересечения
...
> **ПРАВИЛО (2026-02-13):** Никогда не предлагай тактические заплатки без предварительной оценки по 7 характеристикам (ЭМОГССБ). Если фикс слаб по ≥2 характеристикам — СТОП, ищи правильное решение.

- [ ] **АрхГейт ДО ответа:** Таблица ЭМОГССБ включена В ПЕРВЫЙ ответ с решением, не во второй после запроса
